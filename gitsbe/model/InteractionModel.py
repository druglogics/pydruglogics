...
class InteractionModel:
    ...
    def load_sif_file(self, interactions_file: str) -> None:
        ...
        self.__model_name = Util.remove_extension(interactions_file)
        ...
        for interaction in interactions:
            if interaction:
                if '<->' in interaction:
                    ...
                    self.__interactions.extend([Util.parse_interaction(line1), Util.parse_interaction(line2)])
                elif '|-|' in interaction:
                    line1 = interaction.replace('|-|', '|-')
                    line2 = interaction.replace('|-|', '-|')
                    self.__interactions.extend([Util.parse_interaction(line1), Util.parse_interaction(line2)])
                    ...
                    self.__interactions.append(Util.parse_interaction(interaction))
    ...
    def remove_interactions(self, is_input: bool = False, is_output: bool = False) -> None:
        ...
        :return None
        """
        interactions_size_before_trim = len(self.__interactions)
        iteration_trim = 0
        ...
        while True:
            ...
            for i in range(len(self.__interactions) - 1, -1, -1):
                source = self.__interactions[i]['source'] if is_input else None
                target = self.__interactions[i]['target'] if is_output else None

                if target and self.is_not_a_source(target):
                    print(f"Removing interaction (i = {i})  (not source):  {self.__interactions[i]}")
                    self.__interactions.pop(i)
                if source and self.is_not_a_target(source):
                    print(f"Removing interaction (i = {i})  (not target):  {self.__interactions[i]}")
                    self.__interactions.pop(i)

            if interactions_size_before_trim <= len(self.__interactions):
                break
        print(f"Interactions after trim ({iteration_trim} iterations): {len(self.__interactions)}\n")
    ...
    def remove_self_regulated_interactions(self) -> None:
        ...
        """
        for i in range(len(self.__interactions) - 1, -1, -1):
            target = self.__interactions[i]['target']
            source = self.__interactions[i]['source']
            if target == source:
                print(f"Removing self regulation:  {self.__interactions[i]}")
                self.__interactions.pop(i)
    ...
    def build_multiple_interactions(self) -> None:
        ...
        sources = {interaction['source'] for interaction in self.__interactions}
        ...
        self.__interactions = multiple_interaction
    ...
    def get_target(self, index: int) -> str:
        return self.__interactions[index]['target']

    def get_all_targets(self) -> List[str]:
        return [interaction['target'] for interaction in self.__interactions]

    def get_activating_regulators(self, index: int) -> List[str]:
        return self.__interactions[index]['activating_regulators']

    def get_inhibitory_regulators(self, index: int) -> List[str]:
        return self.__interactions[index]['inhibitory_regulators']
    ...
    def __str__(self):
        multiple_interaction = ''
        for interation in self.__interactions:
